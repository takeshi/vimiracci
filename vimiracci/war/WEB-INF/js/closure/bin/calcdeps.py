...
def GetExternsFromOptions(options):
  """Generates the inputs from flag options.
  ...
  if not options.externs_lib:  # Parse stdin
    return [];
  externs = [];
 
  logging.info('Scanning Externs files...')
  files = os.listdir(options.externs_lib)
  
  for root, dirs, files in os.walk(options.externs_lib):
    for file in files:
      if file.endswith(".js"):
        externs.append(root+"/"+file);
  return externs;


def Compile(compiler_jar_path, source_paths, out, flags=None,externs_paths=[]):
...
def main():
  ...
  parser.add_option(
                    ...
                    dest='externs_lib',
                    action='store',
                    help='externs library directory'
                    '')
  ...
  extenrs = GetExternsFromOptions(options)
  ...
  if output_mode == 'script':
    ...
    Compile(options.compiler_jar, deps, out, options.compiler_flags,extenrs)
