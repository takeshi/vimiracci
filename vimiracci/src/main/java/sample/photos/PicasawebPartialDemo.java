...
public class PicasawebPartialDemo {
	 ...
	 */
	private void printAlbumLocation(String uname) throws IOException,
			ServiceException {
		...
		for (GphotoEntry entry : feed.getEntries()) {
			if (entry instanceof GphotoEntry) {
				GphotoEntry poto = (GphotoEntry) entry;
				System.out.println(poto.getTitle().getPlainText());
			}
	 ...
	 */
	private void updateAlbumLocation(String uname) throws IOException,
			ServiceException {
		...
		AlbumEntry entry = service.getEntry(patchQuery.getUrl(),
				AlbumEntry.class);
		...
		AlbumEntry updated = service.patch(new URL(albumEntryUrl), fields,
				entry);
	...
	public static void main(String args[]) throws IOException, ServiceException {
		...
		if (args.length == 2) {
			...
			OUT.println("Syntax: PicasawebCommandLine <username> <password>");
			return;
		} else {
